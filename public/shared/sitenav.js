 ...
 *
 * ── 준비 중인 탭은 누를 수 없다 ─────────────────────────────────────────
 * `href: null` 이면 링크가 아니라 회색 글자로 그린다. 눌러서 빈 화면에 도착하는
 * 것보다 안 눌리는 편이 낫다(허브의 준비 중 칸과 같은 처리).
 *
 * 웹툰이 오래 그 상태였는데 **콘텐츠가 없어서**였다. 2026-08-04 에 작품과 회차가
 * 생겨 전제가 사라졌으므로 열었다. 이 분기는 다음 영역(취향 테스트·리워드)이
 * 붙을 때 다시 쓰인다.
 */
...
const TABS = [
  ...
  // 2026-08-04 에 열었다. 원안은 `view: null` 로 막아 두었지만 그건 **콘텐츠가
  // 없어서**였다 — 작품과 회차가 생겨 그 전제가 사라졌다(webtoon_section_plan 6절).
  { key: "webtoon", label: "웹툰", href: "/webtoon/" },
];
