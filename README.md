...
**⑯ 여기서 그만** 은 기획서 `../reward-minigame-research/plans/2026-07-28/PLAN-07_여기서그만.md` 기준입니다.
한 장씩 뽑아 쌓다가 스스로 멈추는 게임인데, **꽝이 떠도 쌓은 것은 그대로 지급**됩니다 —
잃는 경우가 구조적으로 없습니다. 그렇게만 두면 계속 뽑기의 기대값이 항상 크거나 같아져
'그만'을 누를 이유가 사라지므로, 스스로 멈춘 사람에게만 **1.5배**를 줍니다.
이 배수가 최적 정지 지점을 결정하는 유일한 손잡이이자 포인트 원가 조절 레버입니다.
꽝 확률은 매 라운드 화면에 그대로 씁니다 — 감추면 운 게임이 되고, 공개해야 판단 게임이 됩니다.
...
게임이 15개로 늘었지만 **연동 작업량은 그대로 두 파일**입니다. 아케이드 11종의 트리거는
