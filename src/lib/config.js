 ...
 * 컷 하나가 실제로 57원이고, 8컷 한 편이 798원이다(Y9 §1). 게임 쪽 상수와 달리
...
export const YOURSTORY = {
  ...
  CUT_TIERS: [8, 12, 16],
  // 8컷 상한은 하루에 두 번 올렸다(2026-08-13): 630 → 741 → 798.
  // 상한은 **필수 + 재생성 여유**다 — 필수 (앵커 2 + 컷 8) × 57 = 570원.
  //   630원 = 재생성 1회 · 741원 = 3회 · 798원 = 4회
  // 근거는 실측된 초벌 글자 위반 수다. 파일럿 0813-0001 2건 · 0002 3건 · 0004 4건 —
  // 741원이면 0004 가 한 컷 모자라 막혔고, 798원이면 셋 다 통과한다.
  //
  // **재생성 한도(ys-worker.js `regen_pool`)와 반드시 같이 움직인다.** 741 로 올릴 때
  // 한도를 2 로 두었다가, 예산은 3회를 사 주는데 한도가 2 에서 끊어 한 컷을 못 고쳤다.
  COST_KRW: { 8: 798, 12: 970, 16: 1255 },
  ...
  // heartbeat 가 이만큼 끊기면 **접수를 막는 대신 「예약 접수」로 바꾼다**
  // (2026-08-12 개정 · Y9 §2). 제작은 PC 워커가 하므로 PC 가 꺼진 동안 못 그리는
  // 것은 그대로다 — 바뀐 것은 그때 **문을 닫지 않는다**는 것뿐이다. 이 값은 이제
  // 「지금 만들어지고 있는가」를 화면에 정직하게 표시하는 기준선이다.
  WORKER_STALE_MS: 5 * 60 * 1000,

  // 운영자 알림용 판정선은 **따로 둔다**(2026-08-13). 두 판정의 대가가 다르기
  // 때문이다 — 위의 5분을 당기면 멀쩡한 서비스가 손님에게 「예약으로 받아요」로
  // 보이지만, 이쪽이 이르면 운영자 폰이 한 번 울릴 뿐이다. 그래서 알림만 당긴다.
  //
  // 브리지가 20초마다 신호하므로(`ys_bridge.POLL_SEC`) 4분 침묵은 **12번 연속
  // 실종**이다. 매분 도는 cron 과 합쳐 탐지는 4~5분이 된다.
  ALERT_STALE_MS: 4 * 60 * 1000,
   ...
   *
   * 주문 단위 상한(COST_KRW)만으로는 **폭주를 못 막는다.** 한 건이 798원을 지켜도
   * 하루 376건이 들어오면 30만 원이다. 그래서 상한이 3중이다 — 주문·일·월.
  ...
  /**
   * ✍✍ 트랙별 표시 가격 — **자리만 만든다** (트랙 정본 §1 「가격」 · 결정 19 「가」).
   *
   * 지금 고객 화면에는 가격 표시가 없다(초대 베타 · 티켓제). 그래서 이 자리는
   * **오늘 무엇을 바꾸려고 만든 것이 아니라, 갈라야 할 날 고칠 곳을 한 군데로
   * 정해 두려고** 만든다 — 값을 화면·API 두 곳에서 각각 짓기 시작하면 그때부터
   * 「얼마라고 말했는가」가 배포본마다 달라진다.
   *
   * **두 트랙의 값은 지금 같다**(결정 19 Master 승인). 같은 값을 두 번 적지 않고
   * 하나에서 파생시킨다 — 그래야 「같음」이 우연이 아니라 구조가 된다.
   * 갈리는 날 이 객체 한 곳만 고친다. `null` 은 「표시할 값이 아직 없다」다.
   */
  PRICE_KRW: (() => {
    const SAME = null; // 결정 19 — 현재 값은 동일. 재검토는 원가 표본 5건 시점
    return { ys1: SAME, ys2: SAME };
  })(),
...
/**
 * 카드 11장 중 **배우가 아닌 한 장** (설계서 §1-0 · §2 · 지시서 14).
 *
 * 「AI 추천」은 첫 카드이자 기본값이다.
 * 요소 수는 배우 카드와 같게 유지한다(4요소 규격 · 설정 덤핑 금지).
 *
 * ✍✍ **「이야기 맞춤 인물」은 더 이상 카드가 아니다.** 그 방식은 허브의 YS1
 * 타일이 됐다(지시서 14 · 트랙 정본 §1). 여기에 `track` 필드도 두지 않는다 —
 * 트랙은 진입한 문이 정하므로, 카드가 트랙을 안다고 적어 두면 **같은 규칙을 두
 * 곳에서 재는** 자리가 다시 생긴다(원칙 21).
 */
...
/**
 * ✍✍ 두 진입(문)의 이름과 소개 — **트랙마다 각각** (트랙 정본 §1 「고객 화면」 · 지시서 14).
 *
 * ── 왜 문이 둘인가 ──────────────────────────────────────────────────────
 * 하나의 문 안에서 카드 한 장으로 갈리던 것이 결함이었다 — YS1 이 12분의 1로
 * 앉아 「기존 방식」이 사실상 사라졌다. 앞으로 고객은 **어느 서비스인지 먼저**
 * 고르고 들어온다. 그래서 이름·소개도 각각이다.
 *
 * ── 화면이 지어내지 않는다 ──────────────────────────────────────────────
 * 설계서 §4 대로 문안은 전부 서버가 준다. 허브 타일 두 장도 예외가 아니다 —
 * 타일은 이 서비스의 첫 문장이고, 그것이 배포본마다 다르면 안 된다.
 */
export const YS_ENTRY_TEXT = {
  ys1: {
    icon: "✍",
    name: "너의스토리",
    // 기존 허브 타일에 이미 서 있던 정본 문장이다. 옮겨 적은 것이 아니라 옮겨 온 것
  ...
  ys2: {
    ...
    name: "너의스토리2",
    /**
     * 기획 확정 문안 (회신 25 §3 · 2026-09-04). **개발이 지어낸 문장이 아니다** —
     * 컨셉 정본의 배역 개념(「내 이야기를 이 캐릭터가 연기한다 · 사실은 고객
     * 것이고 캐릭터는 얼굴과 말투만 가져온다」)에서 기획이 뽑았다.
     *
     * YS1 문장과 **주어가 같고 술어만 다르다** — 두 문이 같은 서비스의 두 방식임이
     * 한 줄에서 읽힌다. 「당신의 이야기」가 앞에 오는 이유는 **파는 것이 배우가
     * 아니라 고객의 이야기**이기 때문이다.
     *
     * 화면은 이 값이 없으면 그 줄을 아예 그리지 않는다(빈 줄도 남기지 않는다) —
     * 문안이 비어 있던 기간에 만든 동작이고, 다음 트랙이 늘 때도 그대로 쓴다.
     */
    tagline: "당신의 이야기를, 우리 배우가 연기합니다",
    // 배지는 이미 정본이고 실서비스에 나가 있다(설계서 §1-0 · 지시서 14 「배지 유지 가능」).
